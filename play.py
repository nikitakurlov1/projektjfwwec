...
class Game:
    def __init__(self, root):
        ...
        tk.Button(self.menu_frame, text="Управління", width=15, command=self.show_controls).pack(pady=5)
        tk.Button(self.menu_frame, text="Вихід", width=15, command=self.root.quit).pack(pady=5)
    ...
    def show_controls(self):
        ...
        self.controls_window.title("Управління")
        tk.Label(self.controls_window, text="Пробіл — стрибок\nR — перезапуск", font=("Arial", 14)).pack(padx=20, pady=20)
    ...
    def start_game(self):
        ...
        self.score_text = self.canvas.create_text(10, 10, anchor="nw", text=f"Рахунок: {self.score}", fill=WHITE, font=("Arial", 16))
        self.button_counter = 0
        self.button_counter_text = self.canvas.create_text(10, 40, anchor="nw", text=f"Пробіл натиснуто: {self.button_counter}", fill=WHITE, font=("Arial", 12))
    ...
    def space_pressed(self, event):
        if not self.game_over:
            ...
            self.canvas.itemconfig(self.button_counter_text, text=f"Пробіл натиснуто: {self.button_counter}")
    ...
    def update_game(self):
        ...
        for obs in self.obstacles[:]:
            ...
            if obs.x + obs.width < 0:
                ...
                self.score += 1
                self.canvas.itemconfig(self.score_text, text=f"Рахунок: {self.score}")

            if self.check_collision(self.player.get_coords(), obs.get_coords()):
                self.game_over = True
                self.canvas.create_text(
                    WINDOW_SIZE//2, WINDOW_SIZE//2,
                    text="Гру закінчено! Натисніть R для перезапуску",
